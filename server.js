...
io.on("connection", socket => {
  ...
  function broadCastRoomInfo(room) {
    ...
    console.log("The number of occupants " + Object.keys(room.occupants).length);

    io.in(room.name).emit("occupantsChanged", { occupants: room.occupants });
    socket.to(curRoom.name).broadcast.emit('occupants', {occupants: room.occupants});

    console.log("the number of people " + room.clients);
    socket.to(curRoom.name).broadcast.emit('newClientConnect', room.clients);
    socket.to(curRoom.name).broadcast.emit('sendAllUsernames', {usernames: Array.from(curRoom.usernames)});
  ...
  function leaveRoom(socketId) {
    ...
    if (curRoom.sockets[socketId]) {
      ...
      if (Object.values(curRoom.sockets).filter(s => s.username === username).length === 1) {
  ...
  function cleanRoom() {
    return;
    if (Object.keys(curRoom.occupants).length === 0 || Object.keys(curRoom.sockets).length === 0) {
  ...
  socket.on("disconnect", () => {
    ...
    socket.to(curRoom.name).broadcast.emit("occupantsChanged", { occupants: curRoom.occupants });
    ...
    socket.to(curRoom.name).broadcast.emit('newClientDisconnect', curRoom.clients);

    socket.to(curRoom.name).broadcast.emit('sendAllUsernamesDisconnect', {usernames: Array.from(curRoom.usernames)});
